...
### my_icon_vault/auth/permissions.py
...
### my_icon_vault/database/models.py
...
### my_icon_vault/database/connection.py
...
def process_user():
    ...
    Creates a :class:`~my_icon_vault.auth.users.User` instance and validates
    permissions through :class:`~my_icon_vault.auth.permissions.Role`.
    """
    pass
```
...
def update_profile():
    ...
    Uses :meth:`User.get_profile <my_icon_vault.auth.users.User.get_profile>` 
...
def login_user():
    ...
    Calls :func:`~my_icon_vault.auth.users.authenticate` to verify credentials
    and :func:`~my_icon_vault.auth.permissions.check_permission` for authorization.
    """
    pass
```
...
def setup_auth():
    ...
    Configures components from :mod:`my_icon_vault.auth.users` and 
    :mod:`my_icon_vault.auth.permissions` modules.
    """
    pass
```

**Rendered as**: Configures components from `my_icon_vault.auth.users` and `my_icon_vault.auth.permissions` modules.

### 2. Referencing Elements Within the Same Module

Use short paths when referencing elements in the same Python file:

#### Example: my_icon_vault/auth/users.py
...
:ref:`getting-started` guide, then review the :class:`~my_icon_vault.database.models.BaseModel`
class documentation.

For connection management, see :func:`~my_icon_vault.database.connection.connect`
...
:func:`~my_icon_vault.database.connection.connect`.
...
class BaseModel:
    ...
    through :func:`~my_icon_vault.database.connection.connect`.
    
    Integration with authentication is handled through 
    :class:`~my_icon_vault.auth.users.User` instances. User permissions are
    validated using :func:`~my_icon_vault.auth.permissions.check_permission`.
    ...
    def save(self) -> None:
        ...
        Uses the connection established by :func:`~my_icon_vault.database.connection.connect`.
        Requires authentication through :func:`~my_icon_vault.auth.users.authenticate`.
        """
        pass
    ...
    def find_by_id(self, record_id: int) -> Optional[dict]:
        ...
        .. seealso::
            ...
            :func:`~my_icon_vault.database.connection.Database.query` for custom queries.
        """
        pass
```
...
class UserManager:
    ...
    **Core Components:**
    ...
    - User creation: :class:`~my_icon_vault.auth.users.User`
    - Authentication: :func:`~my_icon_vault.auth.users.authenticate`
    - Authorization: :func:`~my_icon_vault.auth.permissions.check_permission`
    - Data persistence: :class:`~my_icon_vault.database.models.BaseModel`
...
:class:`my_icon_vault.auth.users.User`
